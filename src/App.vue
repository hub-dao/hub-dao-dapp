...
export default {
  ...
  data () {
    return {
      ...
      coinList: [
        ...
        {
          name: 'bnb',
          img: require('./assets/coin_bnb.png'),
          total: '...',
          usd: 0,
          totalBalance: 0,
          status: 0,
        ...
        {
          name: 'gof',
          img: require('./assets/coin_gof.png'),
          total: '...',
          usd: 0,
          totalBalance: 0,
          status: 0,
  ...
  methods: {
    ...
    async getCoinPrice () {
      ...
      let GOF = new Token(ChainId.MAINNET, this.contract.address.uniswap.GOF_WETH, 18)
      ...
      let gof_usdt = eth_usdt / GOFUSDCRoute.midPrice.toSignificant(6)
      ...
      this.coinList.forEach(p => {
        ...
        if (p.name === 'weth') {
          ...
          p.usd = new BigNumber(ht_usdt).toNumber();
        } else if (p.name === 'gof') {
          p.usd = new BigNumber(gof_usdt).toNumber();
        } else if (p.name === 'gxc') {
    ...
    getStartTime () {
      ...
      // this.$store.commit('updateStakeStartTime', 1600862400000);
      // return;
        return new Promise((resolve, reject) => {
    ...
    async getAllRewardRate () {
      ...
      let gof = await this.getRewardRate('gof');
      let weth = await this.getRewardRate('weth');
      let bnb = await this.getRewardRate('bnb');
      let hdt = this.HDTPrice;
      this.coinList.forEach(e => {
        if (e.usd > 0) {
          switch (e.name) {
            case 'gxc':
              e.rate = this.calcAPR(gxc, e.total, hdt, e.usd)
              break;
            case 'ht':
              e.rate = this.calcAPR(ht, e.total, hdt, e.usd)
              break;
            case 'usdt':
              e.rate = this.calcAPR(usdt, e.total, hdt, e.usd)
              break;
            case 'gof':
              e.rate = this.calcAPR(gof, e.total, hdt, e.usd)
              break;
            case 'weth':
              e.rate = this.calcAPR(weth, e.total, hdt, e.usd)
              break;
            case 'bnb':
              e.rate = this.calcAPR(link, e.total, hdt, e.usd)
              break;
            default:
              break;
          }
    ...
    calcAPR (symbol, total, hdt, usd) {
      return (((symbol / (total < 1 ? 1 : total) * hdt) * 360 * 24 * 60 * 60) / usd) * 100;
    },
    changeIcon (key) {
      switch (key) {
        case 'zh-CN':
