    ...
    getBalance () {
      setTimeout(() => {
        this.contract.token.methods.balanceOf(this.address).call().then(result => {
          this.hdtBalance = keepDecimalsDown(new BigNumber(result).div(1e18));
        }).catch(err => {
          errorHandler(err);
        });
      }, 1000)
...
.Wallet {
  ...
  .golBor {
    border: 1px solid;
    border-image: linear-gradient(180deg, rgba(243,112,34,0.06), rgba(243,112,34,0.02)) 1 1;
    border-radius: 28px;
    box-shadow: 0px 12px 7px -10px rgba(243,112,34,0.25); 
  }
  .tips {
    color: #333333;
    margin-bottom: 24px;
